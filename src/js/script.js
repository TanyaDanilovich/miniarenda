...
const swiperHeight = window.innerHeight - header.clientHeight
...
document.querySelector(".hamburger__menu__container").addEventListener('click', () => {
    document.querySelectorAll('.hamburger-line').forEach((el) => {
        el.classList.add('hamburger-line-cross')
    });
...
document.querySelector('.navigation__content').addEventListener('click', () => {
    document.querySelectorAll('.hamburger-line').forEach((el) => {
        el.classList.remove('hamburger-line-cross')
    });
...
mainSwiper.on('afterInit', setTimeout(() => {
